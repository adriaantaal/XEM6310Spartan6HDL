...
always @(posedge clk or posedge rst) begin
	if(rst) begin
		...
		case(state)
			...
			CLEAR: begin //clear mem, this is the returning state when swapping BETA to 1
				dout <= 16'b0;
				req_fifowr <= 1'b0;
				READ_EN <= 1'b1; 
				SPAD_ON_CLK_EN <= 1'b0;
				...
				//request new pattern to start. Because it only request after making a full pattern, the first loop is the zero pattern
				if (pattern_cnt >= pattern_frames) begin
					NEXT_PATTERN	<= 1'b1;
					pattern_cnt 	<= 32'b0;
				end else begin
					NEXT_PATTERN	<= 1'b0;
				...
				//state
				if(en && PATTERN_VALID && (periods_cnt == 0)) begin	
			...
			READ_PIX: begin //send request for data to IC. This is technically a 3-state process (request, sample, send&next)  combined into 2 states
				...
				if (pattern_cnt == 32'b0) begin
				   LOAD <= 1'b1;									//Assert LOAD to flash DIS_LED and PIX_OFF, asserted 1 clock cycle after ADDR, only on the first of the pattern_cnt
				end else begin
				   LOAD <= 1'b0;									
				end			
			...
			NEXT_PIX_AND_FIFO: begin //send data to FIFO, loop to new address
				...
				if(readfromADDR >= 10'd1023) begin //when the final address is surpassed. The READ_PIX state needs still be reached for pixel 1023
					state <= IDLE;
					readfromADDR <= 10'b0;
					//after reading from all pixels, increase the pattern_cnt
